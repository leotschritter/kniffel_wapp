...
object EvaluateStrategy {
  ...
  def fullHouse(data: List[Int]): Int = if (mapToFrequency(data).max == 3 & mapToFrequency(data).min == 2) 25 else 0

  def bigStreet(data: List[Int]): Int = if (mapToFrequency(data).max == 1 & data.max - data.min == 4) 40 else 0

  def smallStreet(data: List[Int]): Int = if (checkBigStreet(data) | data.distinct.size == 4 & data.distinct.max - data.distinct.min == 3 | data.distinct.sum.equals(19) | data.distinct.sum.equals(16)) 30 else 0

  def kniffel(data: List[Int]): Int = if (mapToFrequency(data).max == 5) 50 else 0
